<template>
  <div class="min-h-screen bg-gray-100 p-6">
    <h1 class="text-2xl font-bold mb-4">
      網站導覽樹狀結構-<span class="text-red-600"
        >系統管理者使用請小心操作-只新增系統功能需到角色指定選單</span
      >
    </h1>
    <div class="flex space-x-2">
      <button
        class="bg-blue-900 font-bold px-2 py-1 hover:cursor-pointer rounded-sm text-white"
        @click="dataAppendMain">
        新增主選單
      </button>
      <button
        class="bg-green-900 font-bold px-2 py-1 hover:cursor-pointer rounded-sm text-white"
        @click="dataUpdate">
        修改
      </button>
      <button
        class="bg-red-900 font-bold px-2 py-1 hover:cursor-pointer rounded-sm text-white"
        @click="dataDelete">
        刪除
      </button>
      <button
        ...
        <template v-if="selectedItem">
          ...
          <div class="mb-2">
            <label class="text-white font-semibold block"
              >型態(0:主,1:子) : {{ editForm.type }}</label
            >
          </div>
          <div>
            <label class="text-white font-semibold block"
              >ID:{{ editForm.id }}</label
            >
          </div>
          <div>
            <label class="text-white font-semibold block"
              >父節點ID:{{ editForm.nav_item_id }}
              <input
                v-model="editForm.nav_item_id"
                type="text"
