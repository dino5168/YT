<template>
    <div class="p-4">
      <Caption color="text-gray-900">代碼檔-管理</Caption>
      <!-- 資料操作區塊-->
      <div  v-show="isShow" class="mb-4 p-4 bg-gray-50 rounded-sm">
        <Caption color="text-blue-600">代碼檔-<span :class="operationColor">{{operationState}}</span></Caption>      
        <div class="flex items-center mb-4 w-80">
            <label class="text-sm font-medium mr-3 whitespace-nowrap">選擇代碼分類：</label>
            <ComboBoxApi v-model="combox_selectedValue" api-url="CODES_CATGORY_LIST" placeholder="請選擇代碼分類.." class="flex-1" />
        </div>
         
        <div class="mb-4  bg-gray-50 rounded-sm">
          <div class="grid grid-cols-1 md:grid-cols-4 gap-2 mt-2">
            <FormInput labelcolor="text-blue-800" :placeholder="`輸入${getColumnDescription(col)}...`" v-for="col in editColumns" :key="col" v-model="editForm[col]"
      ...
      <div class="mb-4 p-4 bg-gray-50 rounded-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-2 mt-2">
          ...
          <FormInput labelcolor="text-red-800" :placeholder="`輸入${getColumnDescription(col)}搜尋...`" v-for="col in searchableColumns" :key="col" v-model="searchForm[col]"
            :label="getColumnDescription(col)" />

        </div>
      </div>
      <!-- 資料表格 -->
      <XTable :rows="filteredTableData" :columns="columns" :columnDescriptions="columnDescriptions" @row-click="onRowClick">
         <!-- 操作欄位 -->
         <template #cell-is_active="{ row }">
          ...
          <template #cell-category_name="{ row }">
            <span class="text-yellow-600"> {{ row.category_name || '無分類' }}</span>
          </template>
  ...
  const combox_selectedValue = ref<string | number | null>(null)
  ...
  // 定義 API URL
  // 查詢欄位描述的 API URL
  const schemaUrl = `${baseUrl}/DBQuery/TableSchema/TABLE_SCHEMAT_COMMENT?table_name=codes`
  
  //查詢、 新增、修改、刪除的 API URL 
  const dataUrl = `${baseUrl}/DBQuery/SELECT_CODES_ALL`
  const appendUrl = `${baseUrl}/DBCreate/TABLE_INSERT/INSERT_CODES`
  const updateUrl = `${baseUrl}/DBUpdate/TABLE_UPDATE/UPDATE_CODES` 
  const deleteUrl = `${baseUrl}/DBDelete/TABLE_DELETE/DELETE_CODES`
...
const searchableColumns = computed(() => {
    // 設定可以搜尋的欄位
    return ['category_name','code_value', 'code_name', 'description']
})
...
const editColumns = computed(() => {
  ...
  return ['code_value', 'code_name', 'description','sort_order']
})
...
onMounted(async () => {
  await loadSchema()
  // ✅ 自訂欄位插入
  //columns.value.push('category_name')
  //columnDescriptions.value.push('分類名稱')
  const index = 2
  columns.value.splice(index, 0, 'category_name')
  columnDescriptions.value.splice(index, 0, '分類名稱')
...
const editRow = (row: any) => {
  ...
  isShow.value = true
  combox_selectedValue.value = row.category_id // 設定選擇的分類
  fillEditForm(row)
...
function parseNullableParentId(value: any): number | null {
...
function intToNumber(str:string): number {
  // 假設這個函數返回你的 API 基礎 URL
...
const eventAppend = async () => {
  ...
  const payload = {
      ...editForm.value,
      category_id:combox_selectedValue.value,
      sort_order :  intToNumber(editForm.value.sort_order), // 確保 sort_order 是數字
      parent_code_id: parseNullableParentId(editForm.value.parent_code_id) // 處理可為 null 的欄位
    }
...
const eventUpdate = async () => {
  ...
  const payload = {
    code_id: selectRow.value.code_id,
    ...editForm.value,
    sort_order :  intToNumber(editForm.value.sort_order), // 確保 sort_order 是數字
    category_id:combox_selectedValue.value,
    parent_code_id: parseNullableParentId(editForm.value.parent_code_id) // 處理可為 null 的欄位
