<template>
    <div class="p-4 space-y-4">
        ...
        <div>
            <FormDate class="w-56" label="選取日期" v-model="searchDate" minDate="2025-07-02" maxDate="2025-07-08">
            </FormDate>

            <FormEmail class="w-96" label="電子郵件" v-model="searchEmail" placeholder="請輸入電子郵件...">
            </FormEmail>
            <ButtonBlue @click="onTest">測試</ButtonBlue>
        </div>

        <XTable :columns="columns" :columnDescriptions="columnDescriptions" :rows="rows" @row-click="onRowClick">
            <!-- 自訂 slot：針對 role 欄位放入 ComboBoxSimple -->
            <template #cell-role="{ row }">
                <ComboBoxSimple v-model="row.role_id" :options="items" placeholder="請選擇" class="w-full" />
            </template>
            <!--操作-->
            ...
            <template #cell-avatar_url="{ row }">
                <img :src="row.avatar_url" alt="使用者頭像"
                    class="w-10 h-10 rounded-full object-cover border border-gray-300" v-if="row.avatar_url" />
                <div v-else
            ...
            <template #cell-is_active="{ row }">
                <label class="inline-flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" v-model="row.is_active" class="form-checkbox h-5 w-5 text-green-600" />
...
const get_user_data = async () => {
    try {
        ...
        const excludeFields = ['google_id', 'created_at', 'updated_at', 'role_id']
        const addFields = [
            { role: '角色' },
            { actions: '操作' }
        ...
        const { columns: cols, columnDescriptions: descs } = transform(data, excludeFields, addFields)
        columns.value = cols;
