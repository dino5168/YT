<template>
  <div class="flex flex-col md:flex-row min-h-[600px] bg-gray-300">
    <!-- 左側播放器 -->
    <div
      :class="[
      ...
      <div class="bg-gray-300 shadow-lg overflow-hidden w-full aspect-video">
        <YouTubePlayer
          ref="playerRef"
          :video-id="videoId"
          :subtitles="subtitles"
    ...
    <transition
      ...
      <div
        v-if="showSubtitles"
        class="w-full md:w-1/3 flex flex-col h-140 mt-4 md:mt-0 md:py-4">
        <div
          ...
          <div
            class="flex items-center justify-between rounded-sm flex-shrink-0">
            <!-- 標題 -->
            ...
            <div class="pr-4">
              ...
              <div class="flex items-center gap-1">
                <IconButton tooltip="列印字幕">
                  <template #icon>
                    <svg
                      class="h-5 w-5 md:h-6 md:w-6 text-yellow-400 transition-all duration-200 hover:scale-110 hover:text-yellow-300"
                      fill="none"
                      stroke="currentColor"
                      stroke-width="2"
                      viewBox="0 0 24 24">
                ...
                <IconButton tooltip="關閉字幕" @click="hideSubtitles">
                  <template #icon>
                    <svg
                      class="h-5 w-5 md:h-6 md:w-6 text-red-500 transition-all duration-200 hover:scale-110 hover:text-red-400 hover:rotate-90"
                      fill="none"
                      stroke="currentColor"
                      stroke-width="2"
                      viewBox="0 0 24 24">
    ...
    <transition
      name="slide-in"
      enter-active-class="transition-all duration-400 ease-out delay-300"
      ...
      <div
        v-if="!showSubtitles"
        class="fixed top-1/2 right-0 transform -translate-y-1/2 z-50">
        <button
          @click="showSubtitlesWithAnimation"
          ...
          <!-- 直排文字 -->
          <span
            class="text-sm transition-all duration-200 group-hover:text-blue-100"
...
function handleSeekTo(time: number) {
  ...
  if (typeof playerRef.value.seekTo === "function") {
    ...
    typeof playerRef.value.player.seekTo === "function"
  ) {
