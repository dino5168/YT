...
<template>
    <div class="w-full">
        ...
        <table class="table-auto w-full text-sm text-left text-gray-900 font-bold border border-gray-600 mt-4">
            <thead class="bg-gray-200 text-gray-950">
                <tr>
                    ...
                    ]" @click="handleSort(columns[index])" class="cursor-pointer hover:bg-gray-500 select-none"
                        :style="columnWidths && columnWidths[columns[index]] ? `width: ${columnWidths[columns[index]]}` : ''">
                        <div class="flex items-center justify-between">
                            <span :class="[columns[index] !== 'id' && desc ? 'italic text-blue-800' : '']">
                                {{ desc || '（無描述）' }}
                            </span>
            ...
            <tbody>
                <tr v-for="(row, rowIndex) in currentPageRows" :key="rowIndex"
                    class="border-t border-gray-700 hover:bg-gray-500 cursor-pointer" @click="handleClick(row)">
                    <!-- XTable.vue (修改這段) -->
                    <td v-for="(col, colIndex) in columns" :key="colIndex" :class="[
                        'px-4 py-2 border border-gray-600',
                        col === 'id' ? 'hidden' : ''
                    ]" :style="columnWidths && columnWidths[col] ? `width: ${columnWidths[col]}` : ''">
        ...
        <!-- Pagination controls -->
        <div v-if="rows.length > 0" class="flex justify-between items-center mt-4 text-white">
            ...
            <div class="flex items-center space-x-2 text-sm">
                ...
                <select v-model="pageSize" class="bg-gray-700 border border-gray-600 rounded px-2 py-1">
...
const startRow = computed(() => sortedRows.value.length === 0 ? 0 : (currentPage.value - 1) * pageSize.value + 1)

const endRow = computed(() => Math.min(currentPage.value * pageSize.value, sortedRows.value.length))
