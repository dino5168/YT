<template>
    ...
    <div class="subtitle-list h-full custom-scrollbar overflow-y-auto">
...
const props = defineProps<{
    currentTime: number
    videoId: string
    scrollPosition?: 'top' | 'center' | 'bottom'  // 新增滾動位置選項
    scrollOffset?: number  // 新增滾動偏移量選項
}>()
    ...
    if (activeIndex.value >= 0) {
        nextTick(() => {
            ...
            if (targetElement && scrollContainer) {
                ...
                // 檢查是否需要滾動 - 放寬檢查範圍
                const isVisible = elementRect.top >= containerRect.top + 20 &&
                    elementRect.bottom <= containerRect.bottom - 100

                if (!isVisible) {
                    // 根據設定計算滾動位置
                    let scrollTop = 0
                    const offset = props.scrollOffset || 20

                    switch (props.scrollPosition || 'top') {
                        case 'top':
                            scrollTop = (targetElement as HTMLElement).offsetTop - (scrollContainer as HTMLElement).offsetTop - offset
                            break
                        case 'center':
                            scrollTop = (targetElement as HTMLElement).offsetTop - (scrollContainer as HTMLElement).offsetTop -
                                ((scrollContainer as HTMLElement).clientHeight / 2) + ((targetElement as HTMLElement).clientHeight / 2)
                            break
                        case 'bottom':
                            scrollTop = (targetElement as HTMLElement).offsetTop - (scrollContainer as HTMLElement).offsetTop -
                                (scrollContainer as HTMLElement).clientHeight + (targetElement as HTMLElement).clientHeight + offset
                            break
                    }
